...
import scipy.signal
#import numpy.linalg.qr


def generate_rnn(rng, N, g, tau, dt, max_firing_rate, ninputs=None):
...
"""
  ...
  if ninputs is not None and ninputs > 1:
    A = (rng.randn(N*ninputs)/np.sqrt(1.0)).reshape((N, ninputs))
    #q, r = np.linalg.qr(A, mode='reduced')
    #print(q.shape)
    #print((np.corrcoef(q.T)).shape)
    #print(np.corrcoef(q.T))
    rnn['Bin'] = A
    #print("check orth")
    #print(np.matmul((q.T),q))
    #rnn['Bin'] = (rng.randn(N*ninputs)/np.sqrt(1.0)).reshape((N, ninputs))
  else:
...
def generate_data(rnn, T, E, x0s=None, P_sxn=None, input_magnitude=0.0,
                  input_times=None, ninputs=None, rng=None, lorenz_input_flag=None):
  ...
  """
  N = rnn['N']
  def run_rnn(rnn, x0, ntime_steps, input_time=None, ninputs=None):
    ...
    Bin = (dt/tau*rnn['Bin'])#.reshape((N, ninputs))
    Bin2 = dt/tau*rnn['Bin2']
    b = dt/tau*rnn['b']
    
    if ninputs is None:
      us = np.zeros([N, ntime_steps])
    else:
      us = np.zeros([ninputs, ntime_steps])
    for t in range(ntime_steps):
      x_t = alpha*x_tm1 + np.dot(W,r_tm1) + b
      if input_time is not None:
        if ninputs is not None:
          if ninputs > 1:
            us[:,t] = rng.randn(ninputs)*20
            x_t += np.squeeze(np.matmul(Bin, np.expand_dims(us[:,t], axis=1)))
          if ninputs == 1:
            us[0, t] = rng.randn(1)*20
            x_t += Bin * us[0, t]
        if ninputs is None and t in input_time:
  ...
  for e in range(E):
    if(ninputs is not None and ninputs > 1):
     ...
     # r_nxt, u_uxt = run_rnn(rnn, x0s[:,e], ntime_steps, input_time, ninputs)
    else:
      input_time = input_times[e] if input_times is not None else None
    r_nxt, u_uxt = run_rnn(rnn, x0s[:,e], ntime_steps, input_time, ninputs)
    ...
    if ninputs is not None and ninputs > 1:
      if e == 0:
        ...
        inputs_e = np.concatenate((inputs_e, u_uxt), axis = 0) #shape = (E*ninputs)xntime_steps
    else:
  ...
  if ninputs is not None and ninputs > 1:
    inputs_e = inputs_e.reshape(E, ninputs, ntime_steps)
  S = P_sxn.shape[0]
...
def split_list_by_inds(data, inds1, inds2, ninputs=None):
